...
namespace Microsoft.Extensions.Logging.Testing
{
    public class AssemblyTestLog : IDisposable
    {
        ...
        public ILoggerFactory CreateLoggerFactory(ITestOutputHelper output, string className, [CallerMemberName] string testName = null, DateTimeOffset? utcNow = null)
            => CreateLoggerFactory(output, className, LogLevel.Trace, testName, utcNow);

        public ILoggerFactory CreateLoggerFactory(ITestOutputHelper output, string className, LogLevel minLogLevel, [CallerMemberName] string testName = null, DateTimeOffset? utcNow = null)
            => CreateLoggerServices(output, className, minLogLevel, out var _, testName, utcNow).GetRequiredService<ILoggerFactory>();

        public IServiceProvider CreateLoggerServices(ITestOutputHelper output, string className, LogLevel minLogLevel, out string normalizedTestName, [CallerMemberName] string testName = null, DateTimeOffset? utcNow = null)
        {
            ...
            if (!string.IsNullOrEmpty(_baseDirectory))
            {
                ...
                serilogLoggerProvider = ConfigureFileLogging(testOutputFile, utcNow);
            }
            ...
            serviceCollection.AddLogging(builder =>
            {
                ...
                if (output != null)
                {
                    builder.AddXunit(output, minLogLevel, utcNow);
                }
